...
public class AverageSpeedController {
  ...
  private static class AverageSpeedKeySelector
      implements KeySelector<VehicleReport, AverageSpeedKey> {
    @Override
  ...
  private static class AverageSpeedCollector
    ...
    public void apply(
      ...
      if (time2 - time1 != 0) {
        double avgSpeed = ((endPosition - startPosition) * 2.23694 / (time2 - time1)); // in mph
        if (avgSpeed > 60) {
